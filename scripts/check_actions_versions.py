...
`--write` / `--apply` (a deliberate deviation from this repository's other
checker scripts, which take no arguments): rewrite each outdated pin's `uses:` line in
...
@dataclass
class ActionPin:
    """One well-formed `uses: owner/repo@<40-hex-sha> # vX.Y.Z` step."""

    workflow_file: str
    line_no: int
    indent: str  #: the line's exact leading whitespace/`- ` marker, preserved verbatim by `_rewrite_pin`
    owner_repo: str
    sha: str
    tag: str
...
@dataclass
class Discrepancy:
    """`pin` is outdated: `new_sha`/`new_tag` is what it should resolve to
    instead, for the reason stated in `reason`."""
...
def _discover_pins() -> tuple[list[ActionPin], list[str]]:
    """Every `uses:` step across the workflow files, parsed into an
    `ActionPin` (`owner/repo`, pinned commit SHA, trailing `# vX.Y.Z` tag
