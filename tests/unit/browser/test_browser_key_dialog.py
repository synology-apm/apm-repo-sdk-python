...
class _FakeRepo:
    def __init__(
        ...
        *,
        ...
        # raised for a partial reopen/close failure -- see that method's
        # own docstring. Only meaningful for the ExceptionGroup scenario;
        # unused for the plain KeyVerification/ApmRepoError ones above.
...
async def test_action_verify_with_a_typed_but_unsubmitted_key(wait_until: Any) -> None:
    ...
    async with app.run_test() as pilot:
        ...
        await wait_until(pilot, lambda: repo.received_keys != [], timeout=0.6, interval=0.02)
...
async def test_wrong_key_shows_invalid_and_does_not_dismiss(wait_until: Any) -> None:
    ...
    async with app.run_test() as pilot:
        ...
        await wait_until(pilot, lambda: "invalid" in str(status.render()), timeout=0.6, interval=0.02)
...
async def test_action_cancel_dismisses_with_false() -> None:
    # No test in this file exercises action_cancel (Esc) at all.
    repo = _FakeRepo(KeyVerification(gcm_ok=True, vault_key=b"vault"))
    app = _FakeApp(repo)
    async with app.run_test() as pilot:
        screen = app.screen
...
async def test_on_input_submitted_via_a_real_enter_keypress_verifies(wait_until: Any) -> None:
...
async def test_partial_reopen_failure_notifies_but_still_dismisses_with_true(wait_until: Any) -> None:
    ...
    async with app.run_test() as pilot:
        ...
        screen.query_one("#key-input", Input).value = "userKeyID@dGVzdA=="
        screen.action_verify()
        await wait_until(pilot, lambda: app.screen is not screen, timeout=0.6, interval=0.02)
        assert repo.received_keys == ["userKeyID@dGVzdA=="]


async def test_set_key_error_shows_in_the_status_line(wait_until: Any) -> None:
