...
async def verify(
    ...
    async with opened_repo_or_profile(repo, key, profile=profile, state=state) as repository:
        # verify_reachable()'s progress runs in two stages: an "items"
        # tick while discovering/sizing buckets, then a "bytes" tick per
        # bucket actually checked.
        findings = sorted(await repository.verify(level, progress=verify_meter.update), key=sort_key)

    # Sorted once, here — _render_human's grouping relies on group_findings
    # receiving input already sorted by sort_key.
